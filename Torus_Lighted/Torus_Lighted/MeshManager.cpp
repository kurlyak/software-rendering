//======================================
//	Ed Kurlyak 2023 Lighting Software
//======================================
...
void CMeshManager::Init_Torus(HWND hWnd)
{
	m_hWnd = hWnd;
...
void CMeshManager::Calculate_Torus()
{
	...
	*/
	...
	for (UINT i = 0; i < m_VertCount; i++)
	{
		...
		vector3 Norm;
		Vec3_Mat4x4_Mul(Norm, m_NormalBuff[i], MxTemp2);
		...
		vector3 Vec;
		Vec3_Mat4x4_Mul(Vec, m_VertBuff[i], MxTemp2);
...
void CMeshManager::Draw_Torus ()
{
    ...
    for (UINT i = 0; i < m_TriangleCount; i++)
    {
		...
		vector3 PosLight = vector3(-25.0f, 0.0f, -25.0f);
		vector3 DiffLightColor= vector3(255.0f, 255.0f, 128.0f);

		vector3 LightDir1 = PosLight - Vec1;
		vector3 LightDir2 = PosLight - Vec2;
		vector3 LightDir3 = PosLight - Vec3;

		float Dist1 = Vec3_Len(LightDir1);
		float Dist2 = Vec3_Len(LightDir2);
		float Dist3 = Vec3_Len(LightDir3);

		Vec3_Normalize(LightDir1, LightDir1);
		Vec3_Normalize(LightDir2, LightDir2);
		Vec3_Normalize(LightDir3, LightDir3);
	
		float DotProd1 = Vec3_Dot(Norm1,LightDir1);
		float DotProd2 = Vec3_Dot(Norm2,LightDir2);
		float DotProd3 = Vec3_Dot(Norm3,LightDir3);
		...
		Vec3_Mat4x4_Mul(Vec1, Vec1, MxScreen);
		Vec3_Mat4x4_Mul(Vec2, Vec2, MxScreen);
		Vec3_Mat4x4_Mul(Vec3, Vec3, MxScreen);
		...
		Draw_Color_Triangle(Vec1.x, Vec1.y, Vec1.z,
