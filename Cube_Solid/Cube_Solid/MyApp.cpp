...
LRESULT CALLBACK CMyApp::StaticWndProc(	HWND	m_hWnd,
							UINT	msg,
							WPARAM	wParam,
							LPARAM	lParam)
{
	if ( msg == WM_CREATE )
		SetWindowLong( m_hWnd, GWL_USERDATA, (LONG)(LONG_PTR)((CREATESTRUCT FAR *)lParam)->lpCreateParams);

	CMyApp *MainWndProc = (CMyApp*)(LONG_PTR)GetWindowLong( m_hWnd, GWL_USERDATA );
	if (MainWndProc) return MainWndProc->WndProc( m_hWnd, msg, wParam, lParam );
...
int CMyApp::ProgramBegin(HINSTANCE	hInstance, int nCmdShow)
{
	...
	m_MeshManager.Init_Cube(m_hWnd);
    ...
    while (TRUE)
    {
		...
		m_MeshManager.Calculate_Cube();
        m_MeshManager.Draw_Cube();
